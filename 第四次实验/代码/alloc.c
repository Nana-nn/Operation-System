...
phys_clicks clicks;  /* 请求的内存 */ 
{
  /*hole 空闲链表，按照内存地址的递增顺序列出了内存的各个空闲区域
  不过数据段和栈段的空隙不认为是空闲区，因为它们已经被分配给了进程
  每个空闲区表项有3个字段：空闲区的起始地址，空闲区的长度，一个指针指向下一个链表结点
  地址和长度都是以click为单位。click是1024个字节。该链表是一个单向链表，所以best-fit要从
  头开始找。
  先遍历整个空闲内存块列表，找到最佳匹配的空闲块。一块分配给进程，一块
  给剩余的空闲区，仍然留在空闲链表上，但长度要发生变化。
  如果块的大小正好合适，那么就要调用del_slot，把这个结点从空闲链表中删除
  */
    register struct hole *hp, *prev_ptr, *best, *prev_best;
    phys_clicks old_base,best_clicks;
    int find=0; /*find 用来标识是否找到需要的内存块*/
    do {
      //从头开始找
        prev_ptr = NIL_HOLE;
        hp = hole_head;
        while (hp != NIL_HOLE && hp->h_base < swap_base){
            if (hp->h_len >= clicks){
                if (find == 0){/* 第一次找到合适的块 */
                    best = hp;                      /* 记录当前指针 */
                    prev_best = prev_ptr;           /* 记录当前指针 */
                    best_clicks = hp->h_len;        /* 更新最适块大小 */
                    find = 1;                       /* 第一次找到合适的块 */
                }else if (hp->h_len < best_clicks){ /*找到了一个更小的内存块*/
                    best = hp;                      /* 记录当前指针 */
                    prev_best = prev_ptr;           /* 记录当前指针 */
                    best_clicks = hp->h_len;        /* 更新最适块大小 */
                }} 
    ...
    //把一些像wait(), pause() or sigsuspend()这样的进程swap out
    if (find == 1) 
    {
        //要将找到的空闲块一分为二了，前面一部分clicks单位的是要分配的，后面剩下来的best->h_len要减去clicks，
        //然后其实地址也要往前移clicks
        //最后只要返回old_base
        old_base = best->h_base;                    
        best->h_base += clicks;                     //改变块的起始地址
        best->h_len -= clicks;                      //改变块的长度
        //high_watermark是用来记录被使用的内存的最高的地址，如果超了要更新
        if (best->h_base > high_watermark)
            high_watermark = best->h_base; 
        /* 如果块的大小正好合适，即空洞的大小为0 h_len=0 */
        if (best->h_len == 0)
            del_slot(prev_best,best); 
            //把best对应的块删了，pre_best的下一位指向原来best的下一位
        return(old_base);
    }
    return(NO_MEM);//否则就返回没有内存可以分配
}
